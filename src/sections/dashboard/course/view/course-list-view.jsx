...
const TABLE_HEAD = [
  ...
  { id: 'freeOrPaid', label: 'Type', width: 120 },
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    filtered = filtered.filter(
      (course) =>
        course.title.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        (course.description && course.description.toLowerCase().indexOf(name.toLowerCase()) !== -1)
    );
  }
...
export function CourseListView() {
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      const deletePromises = table.selected.map((id) => dispatch(deleteCourse(id)).unwrap());
      ...
      toast.error(error || 'Failed to delete courses');
    }
